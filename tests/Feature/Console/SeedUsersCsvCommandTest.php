...
class SeedUsersCsvCommandTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $this->artisan('seed:users-csv', ['count' => 10])
            ->assertFailed()
    ...
    public function test_command_creates_users_successfully(): void
    {
        $this->artisan('seed:users-csv', [
            'count' => 10,
    ...
    public function test_command_creates_audit_records(): void
    {
        ...
        // Each user creates 1 CREATE + 6 UPDATE = 7 audit records
        $count = DB::connection('operational')
            ->table('user_audits')
            ->where('tenant_id', $this->tenantId)
            ->count();
